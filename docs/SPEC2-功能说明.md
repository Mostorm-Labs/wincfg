...
双击项目根目录的 `WinConf.exe` 并允许管理员权限，即可在同一个界面中管理本文档列出的 8 个配置模块。界面默认显示 English，可通过右上角语言选择切换为中文。

1. 从左侧选择一个配置模块，先阅读用途说明和注意事项。
2. 在“状态对比”中核对当前值和目标值；“不适用”表示当前 Windows 版本不支持该设置。
3. 如需先确认流程，勾选“仅预演（不修改系统）”再运行。
4. 点击“运行此配置项”只会执行当前选中的模块。
5. 执行结束后查看“运行前”“运行后”“目标值”和“结果”四列；失败时切换到“运行日志”查看原因。
6. 成功运行后点击“恢复此配置项”，即可将该模块恢复到本次运行前记录的快照；恢复完成后同样会显示恢复前后对比。

界面读取和对比的是系统实际状态，而不是仅根据日志判断是否成功。`WinConf.exe` 已包含全部运行脚本，可以单独复制到其他目录或设备运行；启动时产生的临时脚本会在界面退出后自动清理。
